...
class Search extends CI_Controller {
	...
	public function Index($query = NULL, $type = NULL){
		...
		elseif ($this->ion_auth->is_admin()) // remove this elseif if you want to enable this for non-admins
		{
			if (!empty($query) && !empty($type))
				{
					$this->data[$type] = $this->yapps_model->search($querya, $type);
					$page = '_view';
					//$this->data['foreachval'] = $type;
					//var_dump($this->data[$type]);
					$this->data['selected'] = NULL;

				} else {
					$this->session->set_flashdata('message', 'Enter your Query');
				}
		}
		else 
		{
		...
		$this->load->view('aries_admin/apps/search'.$page, $this->data);
